/*
  ...
  This example controls an Analog Devices AD5262 digital potentiometer.
  The AD5220 has 2 potentiometer channels. Each channel's pins are labeled
  A - connect this to voltage
  W - this is the pot's wiper, which changes when you set it
  B - connect this to ground.
 
 The AD5262 is SPI-compatible.  To command it, you send one bit specifying
 which potentiometer channel followed by one byte specifyin the resistance
 value (0 - 255).  
 ...
 based on AD5260Digipot_wClockDivider by Chip Audette
 which is based on Arduino Example AD5206 by Tom Igoe
...
//    Serial.println(" V");
    ...
    if (1) {
      ...
      SPI.transfer(255-level);//  send value via SPI:
      digitalWrite(slaveSelectPin,HIGH);// take the SS pin high to de-select the chip
    }
    
    //wait to allow measurement with voltmeter (scale wait time by the pre-scaler!
    delay(3000/8);  //comment out this line to go as fast as possible
  }
}
